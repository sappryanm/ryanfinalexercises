...
SELECT name, code2, population FROM country WHERE population > 800000;
-- 6. The name, country code, and population of all cities with a population less than one thousand people (11 rows)
SELECT name, code2, population FROM country WHERE population < 2000;
...
SELECT name, continent FROM country WHERE continent IN ('North America', 'South America') EXCEPT SELECT name, continent FROM country WHERE region ='Caribbean';
...
SELECT name, continent, (gnp/population) as per capita GNP FROM country WHERE continent IN ('Europe');
